...
class UinLogin
{
    ...
    {
        $url = 'https://ssl.ptlogin2.qq.com/ptqrshow?s=8&e=0&appid=716027609&type=1&t=0.492909' . time() . '&daid=5&pt_3rd_aid=100384226';
        ...
        if ($qrsig = $match[1]) {
            ...
            return array('saveOK' => 0, 'qrsig' => $qrsig, 'qrcode' => $arr['qrcode']);
        } else {
    ...
    public function getqrpic3rd($daid, $appid)
    {
        ...
        $url = 'https://ssl.ptlogin2.qq.com/ptqrshow?s=8&e=0&appid=' . $appid . '&type=1&t=0.492909' . time() . '&daid=' . $daid;
        if ($daid == 73) $s_url = 'https://qun.qq.com/';
        else if ($daid == 1) $s_url = 'https://id.qq.com/index.html';
        else if ($daid == 377) $s_url = 'https://connect.qq.com/login_success.html';
        else $s_url = 'https://qzs.qq.com/qzone/v5/loginsucc.html';
        $referer = 'https://xui.ptlogin2.qq.com/cgi-bin/xlogin?daid=' . $daid . '&hide_title_bar=1&low_login=0&qlogin_auto_login=1&no_verifyimg=1&link_target=blank&target=self&s_url=' . urlencode($s_url) . '&pt_no_auth=0&appid=' . $appid;
        $arr = $this->get_curl_split($url, $referer);
        preg_match('/qrsig=(.*?);/', $arr['header'], $match);
        if ($qrsig = $match[1]) {
            preg_match('/\((.*?)\)/', $arr['body'], $match);
    ...
    public function qrlogin3rd($daid, $appid, $qrsig)
    {
        ...
        $url = 'https://ssl.ptlogin2.qq.com/ptqrlogin?u1=' . urlencode($s_url) . '&ptqrtoken=' . $this->getqrtoken($qrsig) . '&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=0-0-' . time() . '0000&js_ver=10194&js_type=1&login_sig=&pt_uistyle=40&aid=' . $appid . '&daid=' . $daid . '&';
