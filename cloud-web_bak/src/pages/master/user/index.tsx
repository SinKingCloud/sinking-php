...
import {getUserList, updateUserInfo, updateUserMoney} from "@/services/master/user";
...
import {createWeb} from "@/services/master/web";
import {getDomainConfig} from "@/services/admin/web";
import {getPayLog} from "@/services/master/pay";
import {getLogList} from "@/services/master/log";
...
export default (): React.ReactNode => {
  ...
  const onAddFormFinish = async (values: any) => {
    ...
    return createWeb(values).then((r) => {
    ...
    },
    {
      ...
      render: (text: string, record: any) => {
        return <Dropdown overlay={<Menu>
          ...
          <Menu.Item key={"add_modal"} hidden={record?.is_admin}>
            <a style={{fontSize: "small"}} onClick={() => {
              add?.setFieldsValue({user_id: record?.id, domain: domainConfig['master.domains']?.[0]});
  ...
  return (
    <PageContainer title={false}>
      ...
      <Modal key={"add_web"} width={400} destroyOnClose={true} okButtonProps={{loading: isModalAddWebBtnLoading}}
             ...
             onOk={add.submit} okText={"确 认"} onCancel={() => {
        setIsModalAddWebVisible(false);
        add.resetFields();
      }}>
        ...
        <Form form={money} name="control-hooks" onFinish={onMoneyFinish} labelAlign="right" labelCol={{span: 6}}
          ...
          <Form.Item name={"type"} label="类型" rules={[{required: true, message: "请选择操作类型"}]}>
            <Select placeholder="请选择操作类型">
              ...
              <Select.Option value={1}>扣除</Select.Option>
            </Select>
          </Form.Item>
