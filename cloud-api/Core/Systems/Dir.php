...
class Dir
{
    ...
    public function runFiles($path)
    {
        $arr = array('files' => array(), 'dirs' => array(), 'all' => array());
        $target = array_diff(scandir($path), array('.', '..'));
        $obj = &$this;
        array_walk($target, function ($val, $key) use (&$arr, &$obj, $path) {
            $subTarget = "{$path}/{$val}";
            if (is_file($subTarget)) {
                array_push($arr['files'], "{$path}/" . $val);
            } else if (is_dir($subTarget)) {
                array_push($arr['dirs'], "{$path}/" . $val);
                $arr = array_merge_recursive($arr, $obj->runFiles($subTarget));
            }
        });
        return $arr;
    }
    ...
    {
        $d = scandir($dir);
        $arr = array_diff(is_array($d) ? $d : array(), array('.', '..'));
    ...
    {
        $dirs = self::$dirs;
        $target = strtr(trim($target), array('\\' => '/'));
     ...
     * @return bool
     */
    public function removeAll($s = false)
    {
        $dirs = self::$dirs;
        //进行文件夹排序
