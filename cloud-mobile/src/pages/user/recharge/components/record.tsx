...
export default () => {
    ...
    return (
        <Body title="订单记录" right={<Dropdown menu={{ items }}>
            ...
            {loading && <Skeleton.Paragraph animated/> ||
                ...
                <Card key={user.id} style={{marginBottom:"10px"}} title={<div style={{fontSize:"12px",fontWeight:"normal"}}><Icon type={Recharge} style={{marginRight:"3px"}}/>{user.name}</div>}
                      extra={<div style={{fontSize:"11px",color:"gray"}}>{dayjs(user.create_time).format('YYYY-MM-DD')}</div>}>
                        <span style={{fontSize:"11px",color:"gray"}}>
                            <span style={{fontSize:"16px",fontWeight:"bold",marginRight:"3px",color:"#33cc4b"}}>{user.money}</span>
                            元
                        </span><br/>
