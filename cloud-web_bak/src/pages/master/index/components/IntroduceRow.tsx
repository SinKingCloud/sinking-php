...
import {ChartCard, Field} from '@/components/Other/Dashboard/Charts';
import Yuan from "@/components/Other/Dashboard/utils/Yuan";
  ...
  <Row gutter={24}>
    <Col {...topColResponsiveProps}>
      <ChartCard
        ...
        footer={<Field label="日销售额" value={`￥${numeral(visitData?.slice(-1)?.[0]?.consume_money).format('0,0')}`}/>}
        contentHeight={46}
      >
        <TinyArea
          color="#0051eb"
    ...
    <Col {...topColResponsiveProps}>
      <ChartCard
        ...
        footer={<Field label="日订单量" value={numeral(visitData?.slice(-1)?.[0]?.order_succ_num).format('0,0')}/>}
        contentHeight={46}
      >
        <TinyArea
          color="#27b6a2"
